...
#include "hil_servoing/error_calculation.h"
...
{ // error calculator constructor
    ...
    pub_end_effector_position = nh_.advertise<hil_servoing::TrackedPoints>("/eef_pos", 10);
    pub_error = nh_.advertise<hil_servoing::Error>("/image_error", 10);
}
...
{ // calculate error according to task id
	...
	for (int i = 0; i < task_ids.size(); ++i) {
		...
		switch ((int)task_ids[i]) {
            case 0:
    			skip = 2;
    			sub_vector = slice(centers, start_idx, start_idx + skip); // hil_servoing/vs_utilities.h
...
{	//mono vision eef position publisher 
	//cstephens 19/07/18
	hil_servoing::EndEffectorPoints eef_pts; 
