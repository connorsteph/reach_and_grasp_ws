 ...
 * FILE UTILITIES
 *************************************************************************/
std::string format_output_for_file(sensor_msgs::JointState js, geometry_msgs::PoseStamped ps, int dof)
{
    ...
    output = ss1.str() + " " + ss2.str() + " " + ss3.str() + " " + ss4.str();
    return output;
}
    ...
    try {
        if (boost::filesystem::exists(p)) {
            if (boost::filesystem::is_directory(p)) {
                ...
                int i = 1;
                for (const auto& d : boost::make_iterator_range(boost::filesystem::recursive_directory_iterator{p}, {})) {
                    std::cout << i << ") " << d.path().filename().c_str() << "\n";
                    filenames.push_back(d.path().filename().c_str());
                    ++i;
                }
                return filenames;
            } else {
                ROS_WARN_STREAM("Error: Path exists, but is not a directory\n");
            }
        } else {
            ROS_WARN_STREAM("Error: Directory does not exist\n");
        }
    } catch (const boost::filesystem::filesystem_error& ex) { std::cout << ex.what() << '\n'; }
}

void write_to_file(std::string file_name, std::string msg)
{    
    std::ofstream f(file_name, std::ios::app);
    if (!f) { std::cout << "Error opening file: " << file_name << std::endl; }
    else { 
...
std::vector<Eigen::VectorXd> read_joint_positions_from_file(int dof)
{
    std::vector<Eigen::VectorXd> joints;
    std::string line;
    std::istringstream iss;
    std::string prefix = "/home/froglake/AssistiveRobotics/ros_workspace/src/wam_control/log_data/waypoints/";
    std::ifstream f;
    while (true) {
        std::vector<std::string> filenames = get_directory_files(prefix);
        int idx = (int)double_input(1, filenames.size());
        std::string fname = prefix + filenames[idx-1];
        f.open(fname);
        if (!f) {
            ROS_WARN_STREAM("error: file does not exist"); 
            continue;
        } else { // file is valid
            std::cout << "reading waypoints from " << filenames[idx-1] << std::endl;
            break; 
        } 
    }
    while (f.peek() != EOF) {
        ...
        if (dof == 4) {
            ...
            Eigen::VectorXd v(7);
            iss >> v(0) >> v(1) >> v(2) >> v(3) >> v(4) >> v(5) >> v(6);
            joints.push_back(v);           
        } else { 
            ROS_WARN_STREAM("error: invalid degrees of freedom"); 
            break;
        }
    }
    return joints;
