...
ArmControl::ArmControl(ros::NodeHandle nh) {
	...
	log_filename = "/home/froglake/AssistiveRobotics/ros_workspace/src/wam_control/log_data/continuous/wam_18" + t + ".txt";
	waypoints_filename = "/home/froglake/AssistiveRobotics/ros_workspace/src/wam_control/log_data/waypoints/wam_18" + t + ".txt";
    capture_filename = "/home/froglake/AssistiveRobotics/ros_workspace/src/wam_control/log_data/capture/wam_18" + t + ".txt";
	// log_filename = "/home/laura/AssistiveRobotics/ros_workspace/src/wam_control/log_data/continuous/wam_18" + t + ".txt";
	// waypoints_filename = "/home/laura/AssistiveRobotics/ros_workspace/src/wam_control/log_data/waypoints/wam_18" + t + ".txt";
 //    capture_filename = "/home/laura/AssistiveRobotics/ros_workspace/src/wam_control/log_data/capture/wam_18" + t + ".txt";
    wam_namespace = "";
	initialize_wam_connection();
	ROS_INFO_STREAM("ArmControl: Connected to " << wam_namespace << " with " << dof << " DOF");
	// Arm Services
	gravity_comp_srvs = nh.serviceClient<wam_control::GravityComp>(wam_namespace + "/wam/gravity_comp");
	go_home_srvs = nh.serviceClient<std_srvs::Empty>(wam_namespace + "/wam/go_home");
	hold_jnt_srvs = nh.serviceClient<wam_control::Hold>(wam_namespace + "/wam/hold_joint_pos");
	hold_cart_pos_srvs = nh.serviceClient<wam_control::Hold>(wam_namespace + "/wam/hold_cart_pos");
	hold_ortn_srvs = nh.serviceClient<wam_control::Hold>(wam_namespace + "/wam/hold_ortn");
	hold_ortn2_srvs = nh.serviceClient<wam_control::Hold>(wam_namespace + "/wam/hold_ortn2");
	joint_move_srvs = nh.serviceClient<wam_control::JointMove>(wam_namespace + "/wam/joint_move");
	joint_move_block_srvs = nh.serviceClient<wam_control::JointMoveBlock>(wam_namespace + "/wam/joint_move_block");
	pose_move_srvs = nh.serviceClient<wam_control::PoseMove>(wam_namespace + "/wam/pose_move");
	cart_move_srvs = nh.serviceClient<wam_control::CartPosMove>(wam_namespace + "/wam/cart_move");
	cart_vel_srvs = nh.serviceClient<wam_control::CartVel>(wam_namespace + "/wam/cart_vel");
	ortn_move_srvs = nh.serviceClient<wam_control::OrtnMove>(wam_namespace + "/wam/ortn_move");
	ortn_split_move_srvs = nh.serviceClient<wam_control::OrtnSplitMove>(wam_namespace + "/wam/ortn_split_move");
	force_torque_base_srvs = nh.serviceClient<wam_control::ForceTorqueTool>(wam_namespace + "/wam/force_torque_base");
	force_torque_tool_srvs = nh.serviceClient<wam_control::ForceTorqueTool>(wam_namespace + "/wam/force_torque_tool");
	teach_motion_srvs = nh.serviceClient<wam_control::Teach>(wam_namespace + "/wam/teach_motion");
	play_srvs = nh.serviceClient<wam_control::Play>(wam_namespace + "/wam/play_motion", true); // true to mark persistent connection -- meaning it may not always be valid
	link_arm_srvs = nh.serviceClient<wam_control::Link>(wam_namespace + "/wam/link_arm");
	...
	follow_path_srvs = nh.serviceClient<wam_control::FollowPath>(wam_namespace + "/wam/follow_path");
	get_tool_pose_srvs = nh.serviceClient<wam_control::GetToolPose>(wam_namespace + "/wam/get_tool_pose");
	// ROS Subscribers
...
{ // move joints to desired position in joint space
	wam_control::JointMoveBlock req;
...
{ // move arm with target force and torque with respect to either base or tool
	wam_control::ForceTorqueTool req;
...
bool ArmControl::teach_waypoints_joint()
{ // teach waypoints in joint space
	lock_joint_position(false);
	bool capturing = true;
	std::stringstream ss;
	std::string input = "";
	std::cout << "Move arm to desired waypoint then press 'c' to save position\nPress 'd' to finish and save to file\n";
	while (capturing) {
		...
		if (input[0] == 'c' || input[0] == 'C') {   
			...
			std::cout << "Press 'c' to capture waypoint and 'd' when done\n";
		}
	}
	return true;
}

bool ArmControl::teach_waypoints_tool()
{ // TODO FIX
	lock_joint_position(false);
	bool capturing = true;
	std::stringstream ss;
	...
	while (capturing) {
		while (!std::getline(std::cin, input)) { continue; } // wait for users input
		if (input[0] == 'c' || input[0] == 'C') {   
			ss << previous_pose.pose.position.x << " " 
				<< previous_pose.pose.position.y << " " 
				<< previous_pose.pose.position.z << " ";
			ss << previous_pose.pose.orientation.x << " " 
...
bool ArmControl::teach_motion(std::string path_name) 
{
	...
	wam_control::Teach req;
	req.request.path = path_name;
	return teach_motion_srvs.call(req);
}

bool ArmControl::follow_waypoints(std::string path_name) 
{
...
void ArmControl::log_to_file()
{ // logs joint state and pose information to current file in log folder
	std::string log_msg;
	log_msg = format_output_for_file(previous_joint_state, previous_pose, dof);
	write_to_file(log_filename, log_msg);
}